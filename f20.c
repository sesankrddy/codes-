...
void registerUser() {
    ...
    scanf("%14s", mobile); // Limit to 14 chars
    printf("Enter Email: ");
    scanf("%49s", email);
...
void addMoney(struct User* user) {
    ...
    printf("Wallet updated. New balance: Rs %.2f\n", user->walletBalance);
}

// Function to check wallet balance
void checkBalance(struct User* user) {
    printf("Wallet Balance: Rs %.2f\n", user->walletBalance);
}
...
void prepaidRecharge(struct User* user) {
    ...
    if (processPayment(user, amount, modes[mode-1])) {
        ...
        printf("Recharge successful. Cashback: Rs %.2f\n", cashback);
    } else {
...
void billPayment(struct User* user, char* service) {
    ...
    if (processPayment(user, amount, modes[mode-1])) {
        ...
        printf("Payment successful. Cashback: Rs %.2f\n", cashback);
    } else {
        addTransaction(user, "Bill Payment", service, amount, "Failed", 0.0);
    }
}

// Function to show transaction history with filtering
void showHistory(struct User* user) {
    (void)user;
    int filterChoice;
    printf("Filter by: 1. Date 2. Service Type 3. All\n");
    ...
    char filterDate[20] = "";
    char filterService[20] = "";

    if (filterChoice == 1) {
        printf("Enter date (YYYY-MM-DD): ");
        if (scanf("%19s", filterDate) != 1) {
            printf("Invalid input.\n");
            return;
        }
    } else if (filterChoice == 2) {
        printf("Enter service type (e.g., Recharge, Bill Payment): ");
        if (scanf("%19s", filterService) != 1) {
            printf("Invalid input.\n");
            return;
        }
    }
    ...
    for (int i = 0; i < transCount; i++) {
        int match = 1;
        if (filterChoice == 1 && strcmp(transactions[i].date, filterDate) != 0) match = 0;
        if (filterChoice == 2 && strcmp(transactions[i].type, filterService) != 0) match = 0;
        if (match) {
            printf("ID: %d, Date: %s, Time: %s, Type: %s, Service: %s, Amount: Rs %.2f, Status: %s, Cashback: Rs %.2f\n",
...
void monthlyStatement() {
    ...
    float totalAmount = 0.0, totalCashback = 0.0;
    for (int i = 0; i < transCount; i++) {
        if (strncmp(transactions[i].date, month, 7) == 0) {
            printf("ID: %d, Date: %s, Type: %s, Amount: Rs %.2f, Cashback: Rs %.2f\n",
                   transactions[i].transID, transactions[i].date, transactions[i].type, transactions[i].amount, transactions[i].cashback);
            totalAmount += transactions[i].amount;
            totalCashback += transactions[i].cashback;
        }
    }
    printf("Total Amount: Rs %.2f, Total Cashback: Rs %.2f\n", totalAmount, totalCashback);
}
    ...
    while (1) {
        if (!user) {
            ...
            switch (choice) {
                ...
                case 8: showHistory(user); break;
                case 9: miniStatement(); break;
                case 10: monthlyStatement(); break;
                case 11: printf("Logged out.\n"); user = NULL; break;
